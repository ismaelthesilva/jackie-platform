...
import { CheckCircle, ArrowLeft, ArrowRight, Calculator, Brain, TestTube } from 'lucide-react';
...
export default function TestAIPage() {
  ...
  const questions = useMemo<Question[]>(() => [
    ...
    },
    {
      ...
      description: 'You\'ll use this email to complete your account setup later',
      required: true
    },
    {
      id: 'thankYou',
      ...
      description: 'Your profile has been created. Check your email to complete the signup process.'
    }
  ], []);
...
Please try again or contact us if the problem persists.`);
  ...
  const renderQuestion = () => {
    ...
    const handleKeyPress = (e: React.KeyboardEvent, value?: string | number) => {
          ...
          <CardContent className="p-8 text-center">
            <div className="mb-6">
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-4">
                <h3 className="font-semibold text-purple-800 mb-2">Registration Summary:</h3>
                <div className="text-left space-y-1 text-sm">
                  <p><strong>Name:</strong> {answers.name}</p>
                  <p><strong>Age:</strong> {answers.age} years old</p>
                  <p><strong>Email:</strong> {answers.email}</p>
                  <p><strong>Status:</strong> {Number(answers.age) >= 18 ? '✅ Age Verified' : '⚠️ Must be 18+'}</p>
                </div>
              </div>
              ...
              {isLoading && (
                <div className="flex items-center justify-center space-x-2 text-purple-600 mb-4">
                  ...
                  <p className="text-sm">Creating your pre-registration profile...</p>
                </div>
              )}
              
              {emailSent && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <p className="text-green-800 font-medium">🎉 Profile created successfully! Check your email to complete signup.</p>
                  <p className="text-green-600 text-sm mt-1">Use the same email to finish your account setup anytime.</p>
                </div>
              )}
            </div>
          </CardContent>
    ...
    return (
      <Card className="w-full max-w-2xl mx-auto" ref={formRef}>
        <CardHeader className="pb-4">
          ...
          {currentQ.description && <p className="text-sm text-gray-600 mt-2 ml-11">{currentQ.description}</p>}
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-6">
            <Input
              ...
              placeholder={
                currentQ.type === 'email' ? 'your.email@example.com' :
                currentQ.type === 'number' ? 'Enter your answer' :
                'Type your answer...'
              }
              value={answers[currentQ.id] as string || ''}
              onChange={(e) => setAnswers(prev => ({ 
                ...prev, 
                [currentQ.id]: currentQ.type === 'number' ? Number(e.target.value) : e.target.value 
              }))}
            ...
            {currentQ.id === 'age' && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-blue-800 text-sm">
                  � <strong>Age Requirement:</strong> You must be 18 years or older to create an account. Your age helps us personalize your health and fitness recommendations.
                </p>
              </div>
            )}
            ...
            <div className="flex justify-between items-center">
              ...
              <Button 
                ...
                disabled={
                  ...
                  (currentQ.id === 'age' && Number(answers[currentQ.id]) < 18)
                }
                className="px-6 bg-purple-600 hover:bg-purple-700"
              >
                {currentQuestion === questions.length - 2 ? 'Create Profile' : 'Next'}
        ...
        {currentQuestion === 0 && (
          <Card className="max-w-2xl mx-auto">
            <CardContent className="p-6">
              <h3 className="font-semibold text-gray-900 mb-3">🚀 Pre-Registration Benefits:</h3>
              <ul className="space-y-2 text-sm text-gray-600">
                <li>• <strong>Quick Setup:</strong> Save your basic info now, complete signup later</li>
                <li>• <strong>Priority Access:</strong> Get early access to new features and services</li>
                <li>• <strong>Personalized Experience:</strong> We'll customize your dashboard based on your age and preferences</li>
                <li>• <strong>Easy Completion:</strong> Use the same email to finish your full account setup anytime</li>
              </ul>
            </CardContent>
          </Card>
        )}
